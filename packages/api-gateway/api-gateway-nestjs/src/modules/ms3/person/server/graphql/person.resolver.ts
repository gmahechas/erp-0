...
export class PersonResolver
  ...
  onModuleInit(): void {
    this.companyGrpcService = this.moduleRef.get(CompanyGrpcService, {
      strict: false,
    });
    this.typePersonGrpcService = this.moduleRef.get(TypePersonGrpcService, {
      strict: false,
    });
    this.typeIdentificationGrpcService = this.moduleRef.get(
  ...
  company(@Parent() entity: PersonType): Observable<Partial<ICompany>> {
    return this.companyGrpcService
      .searchById({ entity: { id: entity.companyId } })
      .pipe(pluck('entity'));
  }

  @ResolveField(() => TypePersonType)
  typePerson(@Parent() entity: PersonType): Observable<Partial<ITypePerson>> {
    return this.typePersonGrpcService
      .searchById({ entity: { id: entity.typePersonId } })
      .pipe(pluck('entity'));
  }

  @ResolveField(() => TypeIdentificationType)
  typeIdentification(
    @Parent() entity: PersonType,
  ): Observable<Partial<ITypeIdentification>> {
    return this.typeIdentificationGrpcService
      .searchById({ entity: { id: entity.typeIdentificationId } })
      .pipe(pluck('entity'));
  }
}
