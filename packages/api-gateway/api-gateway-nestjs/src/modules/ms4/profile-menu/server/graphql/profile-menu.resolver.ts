...
import {
  ProfileMenuCreateInput,
  ProfileMenuSearchInput,
  ProfileMenuUpdateInput,
  ProfileMenuDeleteInput,
} from '@api-gateway-nestjs/modules/ms4/profile-menu/server/graphql/profile-menu.input';

@Resolver(() => ProfileMenuType)
export class ProfileMenuResolver
  ...
  onModuleInit(): void {
    ...
    this.menuGrpcService = this.moduleRef.get(MenuGrpcService, {
      strict: false,
    });
  }
  ...
  menu(@Parent() entity: ProfileMenuType): Observable<Partial<IMenu>> {
    return this.menuGrpcService
      .searchById({ entity: { id: entity.menuId } })
      .pipe(pluck('entity'));
  }
}
