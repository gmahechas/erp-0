...
import { IEntityService, IEntityOne, IEntityMany, grpcErrors } from '@gmahechas/common-nestjs';

export function BaseGrpcService<T>(packageName: string, serviceName: string): any {
  ...
  abstract class BaseGrpcServiceHost implements OnModuleInit {
    ...
    constructor(
      @Inject(packageName) private readonly grpcClient: ClientGrpc
    ) { }

    onModuleInit(): void {
      this.grpcService = this.grpcClient.getService<IEntityService<T>>(serviceName);
    }
    ...
    searchById(data: IEntityOne<T>): Observable<IEntityOne<T>> {
      return this.grpcService.searchById(data).pipe(
        catchError((error) => throwError(grpcErrors(error.code)))
      );
    }
