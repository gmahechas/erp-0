...
export class TypeIdentificationMongodbService extends BaseMongodbService() {
  constructor(
    @InjectModel(TypeIdentificationDocument.name)
    private readonly entityModel: Model<TypeIdentificationDocument>,
  ) {
    super(entityModel);
  }

  searchMany(
    data: IEntityMany<ITypeIdentification>,
  ): Observable<IEntityMany<ITypeIdentification>> {
    const searchManyAsync = async (): Promise<
      IEntityMany<ITypeIdentification>
    > => {
