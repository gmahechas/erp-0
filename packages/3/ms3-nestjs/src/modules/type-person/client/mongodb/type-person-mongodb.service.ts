...
export class TypePersonMongodbService extends BaseMongodbService() {
  constructor(
    @InjectModel(TypePersonDocument.name)
    private readonly entityModel: Model<TypePersonDocument>,
  ) {
    super(entityModel);
  }

  searchMany(
    data: IEntityMany<ITypePerson>,
  ): Observable<IEntityMany<ITypePerson>> {
